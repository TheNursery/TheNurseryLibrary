...
def lengths(data):
    ...
    if (lens[0] < -10.0*min(abs(lens))):
        lens[0] = -1.0*min(abs(lens))
    if (lens[-1] < -10.0*min(abs(lens))):
        lens[-1] = -1.0*min(abs(lens))

    while max(abs(lens)) > 20*min(abs(lens)):
    ...
    while True:
        ...
        while len(data) < 300000:
            ...
            for j in range(len(dd)/2):
                v = struct.unpack('h', dd[2*j:2*j+2])
                data.append(v[0])
